        ...
        exchangeMutation.mutate({
            ...
            onError: (error: any) => {
                ...
                if (errorCode === 'EXCHANGE_RATE_MISMATCH') {
                    alert('⚠️ 환율이 변동되었습니다. 최신 정보를 반영합니다. 다시 시도해주세요.');

                    queryClient.invalidateQueries({ queryKey: QUERY_KEYS.RATES });
                } else {
                    alert(errorMessage || '환전에 실패했습니다.');
                }
            }
        });
    };
    ...
    return (
        <div className="bg-white p-6 shadow rounded-lg border border-gray-100">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <button
                    type="submit"
                    disabled={exchangeMutation.isPending || !quoteData}
                    className={clsx(
                        ...
                        (exchangeMutation.isPending || !quoteData) && "opacity-50 cursor-not-allowed"
                    )}
                >
                    {exchangeMutation.isPending ? '처리 중...' : '환전하기'}
                </button>
            </form>
        </div>
    );
